function [kernel, interim_latent] = blind_deconv(y, alpha0, alpha1, alpha2, belta_grad, opts)
% 
% Do multi-scale blind deconvolution
%
%% Input:
% @y : input blurred image (grayscale); 
% @alpha: the weight for the edge
% @belta_grad: the weight for the L0 regularization on gradient
% @opts: see the description in the file "demo_deblurring.m"
%% Output:
...
%     end
  ...
  %-----------------------------------------------------------%
  [ks, alpha0, alpha1, alpha2, belta_grad, interim_latent] = blind_deconv_main(ys, ks,...
      alpha0, alpha1, alpha2, belta_grad, threshold, opts); 
   %% center the kernel
